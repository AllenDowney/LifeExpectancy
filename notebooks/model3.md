...
### Step 2.5.5: Create Gap-Only Predictors

```python
# Transform predictors to use only Gap columns (no Male level variables)
# Rationale: In the Male+Gap model, Elastic Net mostly selected Gap variables,
# with only CardioDeathRate_Male as an exception. This suggests that gaps
# are what matter most for predicting HALE gaps, not the absolute levels.
...
# Transform each indicator: create Gap = Male - Female, drop both Male and Female
for indicator in male_female_indicators:
    ...
    if male_col in predictors.columns and female_col in predictors.columns:
        ...
        # Drop both male and female columns (keep only gap)
        predictors = predictors.drop(columns=[male_col, female_col])

print(f"Gap-only predictors: {len(male_female_indicators)} indicators transformed")
print(f"Original columns: {len(predictors_original.columns)}")
print(f"New columns: {len(predictors.columns)}")
print(f"\nSample of gap column names:")
...
decorate(xlabel='Predicted HALE Gap (years)',
         ...
         title='Residuals vs. Predicted Values (Elastic Net Model - Gaps Only)')
```
...
decorate(xlabel='Importance (|Coefficient| × Std)',
         ylabel='Predictor',
         title=f'Top {top_n} Predictors by Importance (Elastic Net Model - Gaps Only)')
```

### Importance by Indicator

```python
# Aggregate importance by indicator (gap importance only)
...
for indicator in gap_indicators:
    ...
    if len(gap_imp) > 0:
        indicator_importance[indicator] = {
            ...
            'Total_Importance': gap_imp[0]  # Same as gap importance in gaps-only model
        }
...
plt.gca().invert_yaxis()
decorate(xlabel='Total Importance (Gap Importance)',
         ylabel='Indicator',
         title='Indicator Importance (Elastic Net Model - Gaps Only)')
```

## Comparison: Gaps-Only vs Male+Gap Models

### Load Male+Gap Model Results for Comparison

```python
# Note: To compare with Male+Gap model, you would need to run model2.md first
# and save the results, or re-run it here. For now, we'll note the comparison
# can be made by running both models and comparing CV R² scores.

print("""
To compare Gaps-Only vs Male+Gap models:

1. Run model2.md to get Male+Gap model results
2. Compare cross-validation R² scores:
   - If gaps-only performs similarly or better, it suggests level effects aren't needed
   - If Male+Gap performs better, level effects (especially CardioDeathRate_Male) matter

3. Compare model complexity:
   - Gaps-only: fewer predictors, more parsimonious
   - Male+Gap: more predictors, but may capture level effects

4. Check if the performance difference justifies the added complexity
""")
```

```python
# Display gaps-only model performance summary
print("\n" + "="*60)
print("Gaps-Only Model Performance Summary")
print("="*60)
