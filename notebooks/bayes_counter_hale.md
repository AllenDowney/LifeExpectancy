...
- Supports multiple model versions (WHO HALE 2019-2021, IHME HALE 2023)

**Model Versions Available**:
- `2019_nocovid`: Pre-COVID baseline (WHO HALE 2000-2019)
- `2021_covid`: Extended analysis (WHO HALE 2000-2021, includes COVID-19)
- `2023_ihme`: **Recommended** - IHME HALE 2000-2023 (methodological consistency, extended coverage)
...
# ============================================================================
# Options:
#   '2019_nocovid' - Pre-COVID baseline (2000-2019, no COVID-19 predictor, WHO HALE)
#   '2021_covid'   - Extended analysis (2000-2021, includes COVID-19 predictor, WHO HALE)
#   '2023_ihme'    - IHME HALE data (2000-2023, includes COVID-19, methodological consistency)
#   'legacy'       - Legacy model (older format, if exists)
MODEL_VERSION = '2019_nocovid'  
MODEL_VERSION = '2021_covid' 
MODEL_VERSION = '2023_ihme'  # ← Current recommended version

# Map model version to suffix
MODEL_SUFFIX_MAP = {
    '2019_nocovid': '_nomid_nogrw_y2019_nocovid',
    '2021_covid': '_nomid_nogrw_y2021_covid',
    '2023_ihme': '_ihme_nomid_nogrw_y2023_covid',  # IHME HALE, 2000-2023
...
print(f"  Data source: {'IHME HALE' if 'ihme' in MODEL_SUFFIX else 'WHO HALE'}")
...
```python
# Load panel dataset
# Note: For IHME model (2023+), use separate panel files; for older models, use combined panel_dataset
if 'ihme' in MODEL_SUFFIX:
...
print(f"Note: This is the gap for the {result['indicator']} predictor, not the HALE gap itself")
...
## Spot Check: Actual vs Predicted HALE Gap for USA (2019)

Verify that the predicted HALE gap matches expectations and compare with the actual value:

```python
# Get actual HALE gap for USA in 2019
usa_2019_mask = (panel_df['country'] == 'USA') & (panel_df['Year'] == 2019)
if usa_2019_mask.any():
    actual_hale_gap = panel_df[usa_2019_mask]['HALE_gap'].iloc[0]
    print(f"Actual HALE gap for USA (2019): {actual_hale_gap:.3f} years")
else:
...
print(f"\nPredicted HALE gap for USA (2019):")
...
output_filename = f'tables/counterfactuals_usa_{latest_year}_hale_bayesian.html'
...
plot_counterfactual_forest(
    counterfactual_results,
    output_prefix=f'counterfactual_effects_usa_{latest_year}_hale',
...
print("Gap-Closing Indicators (negative change = reduces HALE gap):")
...
## Predicted vs Actual HALE Gap Over Time: United States

Plot the predicted vs actual HALE gap over time for the United States to visualize model fit:

```python
# Plot predicted vs actual HALE gap for USA over time
plot_predicted_vs_actual_over_time(
    country='USA',
    trace=trace,
    metadata=metadata,
    ...
    target_name='HALE gap',
    target_col='HALE_gap',
    output_filename='figs/predicted_vs_actual_hale_usa.png'
)
```
...
output_filename = 'tables/positive_contributions_usa_hale_over_time.html'
...
plot_positive_contributions_stacked_area(
    ...
    target_name='HALE gap',
    country='USA',
    output_filename='figs/positive_contributions_stacked_area_usa_hale.png'
)
```
...
plot_positive_contributions_percentage(
    ...
    target_name='HALE gap',
    country='USA',
    output_filename='figs/positive_contributions_percentage_usa_hale.png'
)
```
