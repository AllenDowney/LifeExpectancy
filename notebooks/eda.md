...
log_path = f'logs/eda_{cutoff_year}.txt'
log_file = open(log_path, 'w')

log_file.write("Exploratory Data Analysis Log\n")
log_file.write("=" * 80 + "\n")
log_file.write(f"Notebook: eda.md\n")
...
ax.scatter(comparison_df['WHO_Male'], comparison_df['IHME_Male'], 
           color=AIBM_COLORS['blue'], alpha=0.6, s=50)
# Add diagonal line
...
ax.scatter(comparison_df['WHO_Female'], comparison_df['IHME_Female'], 
           color=AIBM_COLORS['crimson'], alpha=0.6, s=50)
...
ax.scatter(comparison_df['WHO_Gap'], comparison_df['IHME_Gap'], 
           color=AIBM_COLORS['green'], alpha=0.6, s=50)
...
plt.savefig('figs/smoking_scatter.png', dpi=300, bbox_inches='tight')
```

```python
plot_distributions(smoking_gap, indicator_name='SmokingPrevalence')
plt.savefig('figs/smoking_distributions.png', dpi=300, bbox_inches='tight')
```
...
plt.savefig('figs/suicide_scatter.png', dpi=300, bbox_inches='tight')
```

```python
plot_distributions(suicide_gap, indicator_name='Suicide')
plt.savefig('figs/suicide_distributions.png', dpi=300, bbox_inches='tight')
```
...
plt.savefig('figs/alcohol_who_scatter.png', dpi=300, bbox_inches='tight')
```

```python
plot_distributions(alcohol_gap, indicator_name='Alcohol')
plt.savefig('figs/alcohol_who_distributions.png', dpi=300, bbox_inches='tight')
```
...
plt.savefig('figs/poisoning_scatter.png', dpi=300, bbox_inches='tight')
```

```python
plot_distributions(poison_gap, indicator_name='Poisoning')
plt.savefig('figs/poisoning_distributions.png', dpi=300, bbox_inches='tight')
```
...
plt.savefig('figs/road_traffic_who_scatter.png', dpi=300, bbox_inches='tight')
```

```python
plot_distributions(traffic_gap, indicator_name='RoadTraffic')
plt.savefig('figs/road_traffic_who_distributions.png', dpi=300, bbox_inches='tight')
```
...
plt.savefig('figs/maternal_mortality_scatter.png', dpi=300, bbox_inches='tight')
```

```python
plot_distributions(maternal_gap, indicator_name='MaternalMortality')
plt.savefig('figs/maternal_mortality_distributions.png', dpi=300, bbox_inches='tight')
```
...
plt.savefig('figs/homicide_who_scatter.png', dpi=300, bbox_inches='tight')
```

```python
plot_distributions(homicide_gap, indicator_name='Homicide')
plt.savefig('figs/homicide_who_distributions.png', dpi=300, bbox_inches='tight')
```
...
plt.savefig('figs/ipv_scatter.png', dpi=300, bbox_inches='tight')
```

```python
plot_distributions(ipv_gap, indicator_name='IPVPrevalence')
plt.savefig('figs/ipv_distributions.png', dpi=300, bbox_inches='tight')
```
...
plt.savefig('figs/u5mr_scatter.png', dpi=300, bbox_inches='tight')
```

```python
plot_distributions(u5mr_gap, indicator_name='Childhood')
plt.savefig('figs/u5mr_distributions.png', dpi=300, bbox_inches='tight')
```
...
plt.savefig('figs/cardiovascular_who_scatter.png', dpi=300, bbox_inches='tight')
```

```python
plot_distributions(cardio_gap, indicator_name='Cardiovascular')
plt.savefig('figs/cardiovascular_who_distributions.png', dpi=300, bbox_inches='tight')
```
...
plt.savefig('figs/diabetes_who_scatter.png', dpi=300, bbox_inches='tight')
```

```python
plot_distributions(diabetes_gap, indicator_name='Diabetes')
plt.savefig('figs/diabetes_who_distributions.png', dpi=300, bbox_inches='tight')
```
...
plt.savefig('figs/ncd_mortality_scatter.png', dpi=300, bbox_inches='tight')
```

```python
plot_distributions(ncdmort_gap, indicator_name='NCDMortality30_70')
plt.savefig('figs/ncd_mortality_distributions.png', dpi=300, bbox_inches='tight')
```
...
def compare_who_ihme(who_df, ihme_df, who_col, ihme_col, who_label, ihme_label, title):
    ...
    fig_filename = f'figs/who_ihme_comparison_{clean_title}.png'
    ...
    if who_col in who_common.columns and ihme_col in ihme_common.columns:
        ...
        if len(comparison_df) > 0:
            ...
            scatter_filename = f'figs/who_ihme_scatter_{clean_title}.png'
...
```python
# Save to HDF5 file
hdf_file = f'interim/hale_analysis_data_{cutoff_year}.h5'
with pd.HDFStore(hdf_file, mode='w') as store:
    # Cross-sectional data (for Elastic Net models)
    store['predictors'] = predictors
    store['target_hale_who'] = target_hale_who
    store['target_le_who'] = target_le_who
    store['target_hale_ihme'] = target_hale_ihme
    
    # Temporal data (for Bayesian panel models) - cleaned and ready to use
    store['ihme_hale_temporal'] = ihme_hale_temporal  # OECD, 2000-2023
    store['owid_le_temporal'] = owid_le_temporal      # OECD, 2000-2023
    
    # Legacy names (WHO) for backwards compatibility
    store['target_hale'] = target_hale
    store['target_le'] = target_le

log_and_print("\n" + "="*80)
log_and_print("Data saved to HDF5 file")
log_and_print("="*80)
log_and_print(f"File: {hdf_file}")
log_and_print(f"\nCross-sectional datasets (for Elastic Net):")
log_and_print(f"  - predictors: {predictors.shape} (all countries)")
log_and_print(f"  - target_hale_who: {target_hale_who.shape} (includes Turkey)")
log_and_print(f"  - target_le_who: {target_le_who.shape} (includes Turkey)")
log_and_print(f"  - target_hale_ihme: {target_hale_ihme.shape} (excludes Turkey - not in IHME)")
log_and_print(f"\nTemporal datasets (for Bayesian panel models, cleaned):")
log_and_print(f"  - ihme_hale_temporal: {ihme_hale_temporal.shape} (OECD only, 2000-2023)")
log_and_print(f"  - owid_le_temporal: {owid_le_temporal.shape} (OECD only, 2000-2023)")
log_and_print(f"\nLegacy datasets (WHO, backwards compatibility):")
log_and_print(f"  - target_hale: {target_hale.shape}")
log_and_print(f"  - target_le: {target_le.shape}")
log_and_print(f"\nNote: Temporal datasets are pre-filtered to OECD countries and formatted for bayesian_model.md")
log_and_print(f"      Turkey (TUR) is not in IHME HALE but is in OWID LE")
log_file.flush()
```
