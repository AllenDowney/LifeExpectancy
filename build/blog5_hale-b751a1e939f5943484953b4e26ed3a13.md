...
Same “best attainable gap” logic as [Part 3](blog3_counterfactual.md) for LE; parallel HALE run in `notebooks/bayes_counter_hale.md`. **LE counterfactual tables** are not duplicated here (MyST assigns global IDs to included HTML; the same tables appear in Part 3 and would clash). Use Part 3 for the LE gap-extremes and USA-2023 counterfactual **tables**; below we keep HALE-only tables and **alternate LE/HALE figures**.

**Minimum observed rate gaps (LE).**

See table in [Part 3 — The Counterfactual](blog3_counterfactual.md#the-counterfactual).
...
See table in [Part 3 — The United States](blog3_counterfactual.md#the-united-states).
