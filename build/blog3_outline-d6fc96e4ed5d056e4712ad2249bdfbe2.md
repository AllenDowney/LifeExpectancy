...
| Asset | Source | Notes |
|-------|--------|-------|
| **Forest plot (USA)** | `figs/counterfactual_effects_usa_2023_le_bayesian.png` | All indicators with 94% credible intervals; gap-closing vs gap-widening by color |
| **Counterfactual bar chart (USA)** | `figs/counterfactual_effects_usa_2023_le_bar_bayesian.png` | Impact of each intervention, sorted by magnitude; distinguish gap-closing vs gap-widening |
| **Stacked area chart (USA)** | `figs/positive_contributions_stacked_area_usa_le.png` | Positive contributions over time; stacked areas with predicted/actual totals |
| **Counterfactual table (USA)** | `tables/counterfactuals_usa_2023_le_bayesian.html` | Full table with target country-year, credible intervals |
| **Presentation table: gap extremes** | `tables/gap_extremes_min_blog_le.html` | Simplified for blog: minimum gaps by cause, country names, human-readable labels |
| **Presentation table: counterfactuals** | `tables/counterfactuals_usa_2023_le_blog.html` | Simplified for blog: death rate gap, mean only (no intervals); first col "Death rate gap" |
