...
So we'll consider the effect, for a given country, if we can reduce each rate gap to the smallest observed value if it's positive, or zero if it's negative.
...
Road traffic has the largest potential impact — reducing the death rate gap to Iceland's 2017 level would close about 0.81 years of the life expectancy gap.
...
These example show that the leading causes of the life expectancy gap are contingent: they depend on economic and social conditions, and they are amenable to the effect of public health and safety policies.
