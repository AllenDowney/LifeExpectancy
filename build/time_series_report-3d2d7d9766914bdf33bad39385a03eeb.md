...
The following tables show how HALE and Life Expectancy gaps changed for each country over the two-decade period:
...
- Data coverage varies by country and indicator—some countries may have missing years
...
- Slopes are computed using linear regression (`scipy.stats.linregress`) and represent annual change rates
- Superlative analysis identifies extremes by magnitude (absolute value) but displays values with their sign
